...
func (h *handlerV1) GetProductById(c *gin.Context) {
	...
	response, err := h.serviceManager.ProductService().GetProductById(ctx, &pb.ProductId{
		ProductId: id,
	})
...
func (h *handlerV1) DeleteProduct(c *gin.Context) {
	...
	response, err := h.serviceManager.ProductService().DeleteProduct(ctx, &pb.ProductId{
		ProductId: id,
	})
...
func (h *handlerV1) ListProducts(c *gin.Context) {
	...
	response, err := h.serviceManager.ProductService().ListProducts(ctx, &pb.GetAllProductRequest{
		Page:  int64(intpage),
...
func (h *handlerV1) GetPurchasedProductsByUserId(c *gin.Context) {
	...
	response, err := h.serviceManager.ProductService().GetBoughtProductsByUserId(ctx, &pb.UserId{
		UserId: userId,
	})
	if err != nil {
		c.JSON(http.StatusInternalServerError, model.ResponseError{
...
func (h *handlerV1) BuyProduct(c *gin.Context) {
	...
	status, err := h.serviceManager.ProductService().CheckAmount(ctx, &pb.ProductId{ProductId: body.ProductId})
	if err != nil {
		c.JSON(http.StatusInternalServerError, model.ResponseError{
			Code:  ErrorCodeInternalServerError,
			Error: err.Error(),
	...
	_, err = h.serviceManager.ProductService().DecreaseAmount(ctx, &pb.ProductAmountRequest{
		ProductId: body.ProductId,
		Amount:    body.Amount,
	})
	if err != nil {
